...
#'
#'@export
get_an_issue_comment <- function(base_url, api_key, owner, repo, id_issue){
    if (missing(base_url)) {
        ...
        try({
            ...
            content_comment_issue <- content(r, as = "text")
            content_comment_issue <- fromJSON(content_comment_issue)
            return(content_comment_issue)
        })
}