...
test_that("We geta warning when there is no url", {
  expect_warning(get_pull_requests(api_key = api_key, owner = owner,
                                repo = repo),
                 "Please add a valid URL")
})

test_that("We geta warning when there is no api_key", {
  expect_warning(get_pull_requests(base_url = base_url, owner = owner,
                                repo = repo),
                 "Please add a valid API token")
})

test_that("We geta warning when there is no owner", {
  expect_warning(get_pull_requests(base_url = base_url, api_key = api_key,
                                repo = repo),
