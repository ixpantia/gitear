...
test_that("We geta warning when there is no url", {
    expect_warning(get_list_comments_issue(api_key = api_key, owner = owner,
                                        repo = repo, id_issue = id_issue),
                   "Please add a valid URL")
    })

test_that("We geta warning when there is no api_key", {
    expect_warning(get_list_comments_issue(base_url = base_url, owner = owner,
                                        repo = repo, id_issue = id_issue),
                   "Please add a valid API token")
})

test_that("We geta warning when there is no owner", {
    expect_warning(get_list_comments_issue(base_url = base_url,
...
test_that("The comments of issues is read correctly", {
    test_list_comments_issue <- get_list_comments_issue(base_url, api_key, owner,
                                               repo, id_issue)
    expect_true(exists("test_list_comments_issue"))
})

test_that("Obtaining issue comments gives the expected result", {
    value_list_comments_issue <- get_list_comments_issue(base_url, api_key, owner,
                                           repo, id_issue)
