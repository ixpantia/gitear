...
test_that("We geta warning when there is no url", {
    expect_warning(get_org_hook(id = id, org = org, api_key = api_key), "Please add a valid URL")
})

test_that("We geta warning when there is no api_key", {
    expect_warning(get_org_hook(id = id, org = org, base_url = base_url),"Please add a valid API token for the URL you are trying to access")
})

test_that("We geta warning when there is no name of organization", {
    expect_warning(get_org_hook(id = id, base_url = base_url, api_key = api_key),"Please add a valid name of the organization")
})
...
test_that("The hook is read correctly", {
    test_get_org_hook <- get_org_hook(id, org, base_url, api_key)
