...
test_that("We geta warning when there is no url", {
    expect_warning(get_org_hook(id_hook = id_hook, org = org,
                                api_key = api_key), "Please add a valid URL")
})
...
test_that("We geta warning when there is no name of organization", {
    expect_warning(get_org_hook(id_hook = id_hook, base_url = base_url, 
                                api_key = api_key),
                   "Please add a valid name of the organization")
})

test_that("We geta warning when there is no id", {
    expect_warning(get_org_hook(org = org, base_url = base_url,
                                api_key = api_key), 
